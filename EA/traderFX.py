...
class traderFX:
    ## ============================== function core ===================================
    def __init__(self,balance = 200,lot = "standard",leverage = "1:100",spread = 0.00022,mt5_service = False):
        """
    ...
    def send_signal(self,tick,Type,amount = 0.5,order_id = None):
        if Type == "BUY":
...
# data = pd.read_excel('data/Test_data/sin_dataset.xlsx',header=None)
# data.columns = ["time","open","high","low","close"]
# adx = ta.ADX(data.loc[:,'high'],data.loc[:,'low'],data.loc[:,'close'],7)
# print(adx)
data = pd.read_excel('data/TimeFrame/2004/GBPUSD-2004_H1.xlsx',header=None)
data = data.iloc[:,:5]
data.columns = ["time","open","high","low","close"]
ema12 = ta.EMA(data.loc[:,"close"],12)
ema36 = ta.EMA(data.loc[:,"close"],36)
...
#     )
# )
# fig_data2.update_layout(xaxis_rangeslider_visible=False)
# fig_data2.show()
